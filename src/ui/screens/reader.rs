...
/// Visual tone of one reader line; the renderer maps tones to theme styles.
...
pub(crate) fn content(state: &AppState, width: usize) -> Vec<ReaderLine> {
    ...
    lines.push(ReaderLine {
        tone: Tone::Body,
        text: String::from("Reply r · Forward f · Archive e · Star s · Unread u · Delete ⌫"),
    });
    lines.push(hairline(w));

    // Body, wrapped to the viewport (plan §13: reflow at current width;
    // Unicode-safe via ui::text). Missing bodies render explicit
    // placeholders (plan §19 Phase 4 acceptance).
    let body_lines: Vec<String> = match &state.open_message {
        Loadable::Loaded(message) => message_body_lines(message, w.saturating_sub(INDENT.len())),
        Loadable::Failed(detail) => vec![format!(
            "{INDENT}(message could not be loaded — {})",
            first_line(detail)
        )],
        // `Idle` is defensive: a reader route without its load lifecycle.
        Loadable::Idle => vec![format!("{INDENT}(no message loaded)")],
        Loadable::Loading => vec![format!("{INDENT}(loading message…)")],
    };
        ...
        && !message.attachments.is_empty()
    {
        ...
        for attachment in &message.attachments {
            ...
            lines.push(ReaderLine {
                tone: Tone::Dim,
                text: text::truncate(&format!("{INDENT}[ {name} · {mime} · {size} ]"), w),
            });
        }
    }
    lines
...
pub fn render(frame: &mut Frame<'_>, area: Rect, state: &AppState, theme: &Theme) {
    ...
    let width = crate::ui::layout::reader_width((area.width, area.height)).max(10);
    ...
    let visible: Vec<Line<'_>> = lines
        ...
        .skip(start)
        .take(viewport)
        .map(|line| Line::from(reader_span(line, theme, area.width as usize)))
        .collect();
    frame.render_widget(Paragraph::new(visible), area);
}

fn reader_span<'a>(line: &'a ReaderLine, theme: &'a Theme, width: usize) -> Span<'a> {
    ...
    let clipped = text::clip(&line.text, width);
...
fn push_meta(lines: &mut Vec<ReaderLine>, label: &str, value: &str, width: usize) {
    lines.push(ReaderLine {
        tone: Tone::Dim,
        text: text::truncate(&format!("{label:<5} {value}"), width),
    });
}
...
/// Body lines of a fetched message. Plain text wins (Phase 5 brings the
/// HTML rendering subsystem, plan §19); HTML-only mail degrades to a clear
/// note instead of dumping markup.
fn message_body_lines(message: &Message, width: usize) -> Vec<String> {
    match &message.plain_body {
        Some(plain) => text::wrap(plain, width.max(10))
            .into_iter()
            .map(|line| format!("{INDENT}{line}"))
            .collect(),
        None if message.html_body.is_some() => vec![format!(
            "{INDENT}(HTML message — plain-text rendering unavailable)"
        )],
        None => vec![format!("{INDENT}(no content)")],
    }
}
...
mod tests {
    ...
    use crate::domain::{
        Address, Attachment, MailboxId, MessageHeaders, MessageId, MessageSummary,
    };
    ...
    #[test]
    fn html_only_mail_degrades_to_a_note() {
        let mut state = mock::mock_initial_state();
        let summary = summary();
        let message = Message {
            id: summary.id.clone(),
            ...
            headers: MessageHeaders::default(),
            plain_body: None,
            html_body: Some(String::from("<html><body><b>rich</b></body></html>")),
            attachments: Vec::new(),
        };
        state.routes.push(Route::Message(MessageRoute {
            mailbox_id: MailboxId(String::from("inbox")),
        ...
        assert!(lines.iter().any(|l| {
            l.text
                .contains("(HTML message — plain-text rendering unavailable)")
        }));
    }
    ...
    fn loading_and_failed_states_render() {
        ...
        let lines = content(&state, 100);
        assert!(lines.iter().any(|l| l.text.contains("(loading message…)")));
        ...
        assert!(
            lines
                .iter()
                .any(|l| l.text.contains("message could not be loaded") && l.text.contains("boom"))
        );
    }
